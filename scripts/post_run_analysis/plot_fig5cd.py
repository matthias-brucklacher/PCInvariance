...
def load_network_accuracy(resultfolder, architectures, n_ipc_list, decode_mode):
    ...
    for networkIt in ['net_1', 'net_2']:
        ...
        for n_ipcIt in n_ipc_list:
            with open(resultfolder+f'/arch-{architectures[networkIt]}_nipc-{n_ipcIt}/metrics/linear_decoding_accuracy_{decode_mode}_mnist_extended_fast_noise-0_static-0.txt', "rb") as fp:
                list = pickle.load(fp)
                acc_means[networkIt].append(list[-1]) # take value at last training epoch
            with open(resultfolder+f'/arch-{architectures[networkIt]}_nipc-{n_ipcIt}/metrics/linear_decoding_accuracy_{decode_mode}_std_mnist_extended_fast_noise-0_static-0.txt', "rb") as fp:
                list = pickle.load(fp)
...
if __name__ == '__main__':
    ...
    ax2.set_ylabel('Validation decoding accuracy')
    label_fontsize = 16
    ...
    resultfolder = '../results/intermediate/fig5cd'
